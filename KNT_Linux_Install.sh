...
if [ $wsl -eq 1 ]; then
    echo 'This script will install a lot of neuro-imaging software on your WSL2'
else  
    echo 'This script will install a lot of neuro-imaging software on a Linux system'
fi
if [ ! -f ${install_location}/.KUL_apps_install_yes ]; then
...
then
    echo 'OK we continue'
    touch ${install_location}/.KUL_apps_install_yes
else
    exit
fi
fi
...
if [ $wsl -eq 1 ] && [ ! -f ${install_location}/.KUL_apps_install_wsl2_yes ]; then
    echo -e "\n"
    ...
    then
        echo 'OK we continue'
        touch ${install_location}/.KUL_apps_install_wsl2_yes
    else
        exit
    fi
fi
