...
clear
if [ $wsl -eq 1 ];then
    echo 'This script will install a lot of neuro-imaging software on you WSL2'
else  
...
then
    ...
    if [ $wsl -eq 1 ];then
        echo 'Now exit WSL2-Ubuntu and run the KNT_Linux_install.sh again from terminal'
    else
...
then
    ...
    ./set_path
    #echo "" >> $HOME/.bashrc
    #echo "# scaling for mrview" >> $HOME/.bashrc
    #echo "export QT_SCALE_FACTOR=1.5" >> $HOME/.bashrc
    #echo "export GDK_SCALE=1.5" >> $HOME/.bashrc
    cd
    source .bashrc
else
...
then
    ...
    echo "export FREESURFER_HOME=\$HOME/KUL_apps/freesurfer" >> $HOME/.bashrc
    echo "export SUBJECTS_DIR=\$FREESURFER_HOME/subjects" >> $HOME/.bashrc
    echo "export FS_LICENSE=\$HOME/KUL_apps/freesurfer_license/license.txt" >> $HOME/.bashrc
    echo "source \$FREESURFER_HOME/SetUpFreeSurfer.sh" >> $HOME/.bashrc
    sudo apt-get -y install tcsh
    mkdir -p $HOME/KUL_apps/freesurfer_license
    echo "Install the license.txt into /KUL_apps/freesurfer_license"
    rm ${freesurfer_version1}.tar.gz
else
    echo 'Already installed Freesurfer ${freesurfer_version2}'
    echo '  however do not forget to install the license.txt into /$HOME/KUL_apps/freesurfer_license'
fi
...
then
    ...
    echo "export PATH=/usr/local/MATLAB/R2017b/bin:\$PATH" >> $HOME/.bashrc
