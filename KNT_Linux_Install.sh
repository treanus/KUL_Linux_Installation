...
if [[ $(grep microsoft /proc/version) ]]; then
    ...
    echo " be sure to enable nvidia cuda in WSL following https://docs.microsoft.com/en-us/windows/ai/directml/gpu-cuda-in-wsl "
    echo " otherwise, exit the script and do this first"
    sleep 4
    echo -e "\n\n\n"
fi


# search on which distribution of Linux you are
...
#sudo apt update
#sudo apt upgrade
sudo apt -y install git libgl1-mesa-glx libegl1-mesa libxrandr2 libxrandr2 libxss1 libxcursor1 libxcomposite1 libasound2 libxi6 libxtst6 \
    g++ zlib1g-dev \
...
echo -e "\n\n\n"
if ! [ -d "${install_location}/.KUL_apps_useful_installed" ]
then
    cat <<EOT >> ${KUL_apps_config}
...
EOT
    ...
    echo 'Already set up some useful stuff'
fi
...
then
    ...
    read -r -p "Do you want to recompile to SPM binaries (if so you need matlab installed) [y/n] " prompt
    if [[ $prompt == "y" || $prompt == "Y" || $prompt == "yes" || $prompt == "Yes" ]]
    then
        echo 'OK we continue'
        cd spm12/src
